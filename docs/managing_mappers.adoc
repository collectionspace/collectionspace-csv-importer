...
`cspace-config-untangler` is also used to generate a mapper manifest JSON file, which gives details on the profile, version, status, and location of each mapper to be included in csvspace.
...
- Put the file at some location with a URL your dev instance of `csvspace` can access
- Update `config.mappers_url` in `config/application.rb` with the URL of the mapper manifest you wish to use.
- In base of `csvspace` directory, do the following to open the app in the console:
